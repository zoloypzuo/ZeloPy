...
class HexView(QtGui.QFrame):
		...
		if selected_rect_info:
			...
			if (self.selected_region_mode == HexView.SELECTED_REGION_BLOCK) or (begin_line_index == end_line_index):
				...
				painter.drawRect(QtCore.QRect(begin_byte_rect.topLeft(),
											  QtCore.QSize(
												  end_byte_rect.x() - begin_byte_rect.x() + end_byte_rect.width(), \
				...
				painter.drawRect(QtCore.QRect(begin_byte_rect.topLeft(),
											  QtCore.QSize(
												  begin_line_rect.x() + begin_line_rect.width() - begin_byte_rect.x(),
												  begin_line_rect.height())))
				painter.drawRect(QtCore.QRect(end_line_rect.topLeft(),
											  QtCore.QSize(
												  end_byte_rect.x() + end_byte_rect.width() - end_line_rect.x(),
												  end_line_rect.height())))
	...
	def selecting_end_selected_region(self, pos):
		...
		current_begin_selected_byte_pos = min(self.selecting_begin_selected_byte_pos,
											  self.selecting_end_selected_byte_pos)
		current_end_selected_byte_pos = max(self.selecting_begin_selected_byte_pos,
											self.selecting_end_selected_byte_pos)
	...
	def copy_selected_data(self):
		if self.selected_region_mode == HexView.SELECTED_REGION_HIDDEN:
			return
		selected_data = self.data_source.read(self.end_selected_byte_pos - self.begin_selected_byte_pos + 1,
											  self.begin_selected_byte_pos)
		if self.view_mode == HexView.VIEW_HEX:
